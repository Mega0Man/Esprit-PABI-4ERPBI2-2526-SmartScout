...
def run_pipeline(db_url=None):
    ...
    experiment_name = "Retention_Classification"
    mlflow.set_experiment(experiment_name)

    print("=" * 60)
    print("CLASSIFICATION - AUTOMATED ML TRAINING PIPELINE")
    print("=" * 60)
    ...
    with mlflow.start_run(run_name=pipeline_run_name) as pipeline_run:
        ...
        print("-" * 60)
        classification_result = train_classification(db_url, "classification_v1")
        mlflow.log_param("classification_run_id", classification_result["run_id"])
        ...
        print(f"Accuracy        : {classification_result['accuracy']:.4f}")
        print(f"F1 Score        : {classification_result['f1_score']:.4f}")
