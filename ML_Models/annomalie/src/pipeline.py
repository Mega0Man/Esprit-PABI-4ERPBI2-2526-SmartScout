...
try:
    from .train_anomaly import train_anomaly
except ImportError:
    from train_anomaly import train_anomaly
...
def run_pipeline(db_url=None):
    ...
    print("=" * 60)
    print("ANOMALY DETECTION - AUTOMATED ML TRAINING PIPELINE")
    print("=" * 60)
    ...
    with mlflow.start_run(run_name=pipeline_run_name) as pipeline_run:
        ...
        print("\n[1/1] Training Anomaly Detection Model...")
        print("-" * 60)
        anomaly_result = train_anomaly(db_url, "anomaly_v1")
        mlflow.log_param("anomaly_run_id", anomaly_result["run_id"])
        ...
        print(f"Anomaly Bounds  : [{anomaly_result['lower_bound']:.2f}, {anomaly_result['upper_bound']:.2f}]")
